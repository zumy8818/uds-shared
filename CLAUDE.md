...
例：
## 2026-04-16
- HACCP自動化: カフェの温度記録をフォームから自動集計できないか
```
...
```
C:\dev\
├── _ideas\          ← アイデアログ置き場（idea_log.md）
├── _archive\        ← 完了・休止プロジェクトの退避先
├── uds-web-hearing\ ← ウェブ制作ヒアリングシート
├── uds-app-website-generator\ ← LP自動生成システム
└── （新規プロジェクトは uds-shared のGitHub管理ルールに従って命名）
```

### ファイル命名規則
- プロジェクトフォルダ名：`[client-abbrev]-[type]-[date]-[content]`
...
### 稼働中Webサイト
- 配置場所: `C:\dev\internal\active\`
- フォルダ名: ドメイン名をそのまま使う（例: `ubuyama-ds.com`）
- GitHubリポジトリと1対1で対応させる

### 新規プロジェクトをcloneする時の必須手順
1. `cd C:\dev\internal\active`
2. `git clone リポジトリURL ドメイン名`

> ※ `C:\dev\` 直下には絶対にcloneしないこと
...
- 関連フォルダ：`C:\dev\uds-daily-report\`

### 🥉 Phase 3：情報整理・メモ自動化
**目標：アイデアメモが自動で整理・分類される**
- [ ] idea_log.md の定期レビュー仕組み化
- [ ] NotebookLM / Claude API でカテゴリ分類
- 関連フォルダ：`C:\dev\_ideas\`

### Phase 4以降（将来）
- SNS投稿の半自動化（Instagram / Google Business Profile）
- 在庫管理・棚卸アプリ
- HACCPカフェ管理システム
...
作業を通じて「今までよりこちらの方が良い」と気づいたことがあれば、Claudeは積極的にCLAUDE.mdの改善を提案する。

- より良いフォルダ構成・命名規則を発見したとき
- 作業効率が上がるルールに気づいたとき
- 武史さんの作業パターンに合わせた改善案があるとき
...
*最終更新：2026-05-04*
*このファイルは C:\Users\scare\.claude\CLAUDE.md に保存してください*
