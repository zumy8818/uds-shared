...
2. **依頼の「最初の一歩」をすり合わせる（最重要）**
   武史さんの依頼は、本人も気づいていない前提（本当に欲しいゴール）を含んでいることが多い。
   特に依頼が曖昧・大きい場合、Claudeはいきなり本格実装（システム構築・自動化）に入らず、
   まず「これから何をするか」を武史さんの言葉で言い換えて提示し、確認を取ってから着手する。
   例：「MEOキャンペーンのシステムを作って」→ 本当の一歩目は「資料からMEOに必要な要素を
   抽出し、武史さんが理解・営業で使える形にすること」であり、フォーム・DB・自動化構築は
   その次の話だった（2026-07-03の実例）。
   すり合わせを飛ばすと、技術的には正しくても武史さんの目的とズレた作業になり、
   お互いの時間を無駄にする。
3. **前回の作業はどこまで進んだか？**（未完了タスクの確認）
4. **今日の作業はどのプロジェクトか？**（C:\dev\ 配下のどのフォルダか）
5. **どちらのPCか？**（HomePC / ShopPC）
6. **uds-sharedをpullしてCLAUDE.mdを最新化する**
```bash
   cd "C:\coo\スキル・ルール\uds-shared"
   git pull origin main
   copy "C:\coo\スキル・ルール\uds-shared\CLAUDE.md" "$env:USERPROFILE\.claude\CLAUDE.md"
```
7. **`C:\coo\.env` を確認し、作業に必要なキーが揃っているか確認する**（→ 詳細は「14. .env管理ルール」参照）

> Claudeは武史さんが作業開始を宣言したら、上記7点（特に2番目のすり合わせ）を確認してから作業に入ること。
...
```
**エントリを分けるか `pages` に足すかの線引き（重要）**：判断基準は「同一ドメインか」ではなく **「独立して稼働する単位か」**。

- **独立したリポジトリを持ち単独で稼働するツール／アプリ**（例：`/tasks/`＝`uds-task-manager`、`/receipt-ai/`＝`uds-receipt-ai`、`/lp/adiyoshi-koubou/`＝`adiyoshi-koubou-lp` のように、専用リポジトリ・専用デプロイを持つもの）
  → **同じドメイン配下でも、独立した新規エントリとして登録する**
- **同一リポジトリ内の静的な下層ページ**（例：`uds-website` リポジトリ内の会社概要・サービス紹介・キャンペーンページなど、既存サイトに足しただけのページ）
  → **新規エントリを作らず、そのサイトの既存エントリの `pages` 配列に追記する**（＝元ルールの意図「下層ページでエントリを乱造しない」を維持）
- 別ドメインの新規サイトは、新しいエントリを追加する
- 同じ`id`が既にある場合は上書き（重複登録しない）

> 迷ったら **「専用リポジトリ／専用デプロイを持つか？」** で判断する。持つなら独立エントリ、持たない下層ページなら `pages` 追記。
